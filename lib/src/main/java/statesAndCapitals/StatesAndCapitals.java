...
public class StatesAndCapitals
{
    ...
    public void testStatesAndCapitals() throws IOException
    {
        ...
        StateInfo cardinalState = states.stream().filter(bird -> Objects.equals(bird.getStateBird(), "cardinal")).findFirst().get();
        ...
        Boolean isAnyStateLessThan0Elevation = states.stream().anyMatch(stream -> stream.getLowestElevationInFeet() < 0);
        ...
        Boolean isAnyStateGreaterThan21000Elevation = states.stream().anyMatch(stream -> stream.getHighestElevationInFeet() > 21000);
        ...
        Boolean doAllStatesHaveAnAnthem = states.stream().noneMatch(stream -> Objects.equals(stream.getStateAnthem(), null));
        ...
        Integer maxStateElevation = states.stream().map(StateInfo::getHighestElevationInFeet).max(Comparator.naturalOrder()).get();
        ...
        LocalDate earliestDateStateEnteredUnion = states.stream().map(StateInfo::getDateAdmittedToUnion).min(Comparator.naturalOrder()).get();
